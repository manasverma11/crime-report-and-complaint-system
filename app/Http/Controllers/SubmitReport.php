...
class SubmitReport extends Controller
{
    ...
    public function Show_numberRobbery(Request $request) {
        ...
        foreach ($users as $key => $value) {
            ...
            // $userResults = DB::select(DB::raw("SELECT rand FROM robberies WHERE first_name='$val'"));
            $userResults = DB::select("SELECT rand FROM robberies WHERE first_name = ?", [$val]);

            // Merge results if needed, or just assign if you only need the last result
            $results = array_merge($results, $userResults); // Store all results
        }
    
        // If you just want the last user's result, you could do:
        // $results = $userResults; // or whatever logic you prefer
    ...
    public function FindbyRand_robbery(Request $request){
        ...
        $results = DB::select("SELECT * FROM robberies WHERE rand = ?", [$someVariable]);
    ...
    public function showNumber_police(Request $request) {
        ...
        foreach ($users as $key => $value) {
            ...
            // Store each result in the results array
            $result = DB::select(DB::raw("SELECT rand FROM police WHERE name='$val'"));
    ...
    public function Show_numberOffender(Request $request) {
        ...
        foreach ($users as $key => $value) {
            ...
            // Fetch the random value from the database and append it to the results
            $results[] = DB::select(DB::raw("SELECT rand FROM offenders WHERE name='$val'"));
        }
